...
"""
function groebner_basis(polynomials::AbstractVector{M}; max_degree=Inf) where M <: AbstractModuleElement
...
function groebner_basis(G::AbstractVector{M}; kwds...) where M<:AbstractNamedModuleElement{NP} where NP<:NamedPolynomial
    res, tr = groebner_basis(map(_unpack,G); kwds...)
